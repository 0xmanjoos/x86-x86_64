; a semi functional addition calculator that reads from command line
; int argc, char* argv[] is achieved with reading argc from the stack and iterating through argv
; we decrement argc, in this case ecx our counter register because we do not want argv[0], which
; in this case would be something like ./a.out
; this also utilizes an 8 bit register, which is needed in this case since our atoi subroutine only
; copies one ascii byte, which is 8 bits, its explained later on in more detail
;
; nasm -f elf32 08.asm ; ld -m elf_i386 08.o
; usage: ./a.out 10 10 10
; 30
